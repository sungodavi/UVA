...
public class p11512
{
		...
		while(times-- > 0)
		{
			...
			for(int i = 1; i < lcp.length; i++)
			{
				if(lcp[i] >= best)
				{
					...
					best = lcp[i];
					int j = i + 1;
					while(j < lcp.length && lcp[j] == best)
					{
						temp++;
						j++;
					}
					
					if(best > prev || temp > count)
					{
						count = temp;
						index = i;
					}
			...
			if(best == 0)
				...
				out.printf("%s %d\n", t.substring(sa[index], sa[index] + best), count);
		}
		out.close();
	}
	...
	public static void lcp()
	{
		...
		for(int i = 0; i < sa.length; i++)
		{
			...
			{
				while(t.charAt(i + len) == t.charAt(phi[i] + len))
					len++;
				temp[i] = len;
